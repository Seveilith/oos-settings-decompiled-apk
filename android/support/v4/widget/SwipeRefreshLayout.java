  ...
  implements NestedScrollingParent, NestedScrollingChild
{
  ...
  private void finishSpinner(float paramFloat)
  {
    ...
    Animation.AnimationListener local7 = null;
    if (!this.mScale) {
      local7 = new Animation.AnimationListener()
      {
        public void onAnimationEnd(Animation paramAnonymousAnimation)
        {
  ...
  private void moveSpinner(float paramFloat)
  {
    ...
    if (this.mUsingCustomStart)
    {
      ...
      float f3 = i;
      f4 = Math.max(0.0F, Math.min(f4 - f5, 2.0F * f3) / f3);
    ...
    for (;;)
    {
      ...
      this.mProgress.setProgressRotation((0.4F * f2 - 0.25F + 2.0F * f4) * 0.5F);
    ...
    if (Build.VERSION.SDK_INT < 14)
    {
      if ((this.mTarget instanceof AbsListView))
      {
        ...
        if (localAbsListView.getChildCount() > 0) {
          return (localAbsListView.getFirstVisiblePosition() > 0) || (localAbsListView.getChildAt(0).getTop() < localAbsListView.getPaddingTop());
        }
        return false;
      }
  ...
  public boolean onStartNestedScroll(View paramView1, View paramView2, int paramInt)
  {
    ...
    while ((this.mRefreshing) || ((paramInt & 0x2) == 0)) {
      return false;
    }
    return true;
  }
...
/* Location:              C:\Users\johan\Desktop\classes-dex2jar.jar!\android\support\v4\widget\SwipeRefreshLayout.class
